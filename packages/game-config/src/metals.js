...
exports.PLANET_DROP_TABLE = void 0;
const sectors_1 = require("./sectors");
function clampDropChance(chance) {
    return Math.max(0.02, Math.min(0.35, Number(chance.toFixed(3))));
}
function generatedDropsForPlanet(planetId) {
...
function generatePlanetDropTable() {
    const result = {};
    for (let planetId = 16; planetId <= sectors_1.TOTAL_PLANETS; planetId++) {
        result[planetId] = generatedDropsForPlanet(planetId);
    }
    return result;
}
function validatePlanetDropTable(table) {
    for (let planetId = 1; planetId <= sectors_1.TOTAL_PLANETS; planetId++) {
...
const PLANET_DROP_TABLE_HARDCODED = {
    // Sector 1
...
exports.PLANET_DROP_TABLE = validatePlanetDropTable({
