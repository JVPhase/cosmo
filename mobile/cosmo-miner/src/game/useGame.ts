...
function computeInitialShownUnlocks(initial?: GameStateInit): Set<string> {
  ...
  if (titan > 0) { shown.add("metal_titan"); shown.add("ship_cruiser"); shown.add("cannon_titan"); }
  if (iridium > 0) { shown.add("metal_iridium"); shown.add("ship_dreadnought"); shown.add("cannon_iridium"); }
  if (iron > 0 && titan > 0 && iridium > 0) { shown.add("ship_flagship"); shown.add("cannon_alloy"); }
  return shown;
}
...
export function useGame(initial?: GameStateInit) {
  ...
  const totalDamage = useMemo(
    () => Math.floor(computeBaseShipDamage(state.fleet) * derived.damageResearchMultiplier),
    [state.fleet, derived.damageResearchMultiplier]
  );

  const playerLevel = useMemo(() => computePlayerLevel(state.playerXP), [state.playerXP]);
  ...
  useEffect(() => {
    ...
    if (!hasTimed) return;
    const interval = setInterval(() => setNow(Date.now()), state.battle ? 50 : 1000);
    return () => clearInterval(interval);
  }, [!!state.battle, state.expeditions.length]);

  const timeRemaining = state.battle ? Math.max(0, state.battle.expiresAt - now) : 0;
  ...
  useEffect(() => {
    ...
    if (titan > 0) {
      enqueue("metal_titan", {
        ...
        text: "Зафиксирован образец Титана™! Материал группы IV-B, исключительная прочность.\n\nПо регламенту подлежит немедленной конфискации в пользу МГМР. Форма КНФ-3 на рассмотрении с 2379 года. Пока — считайте его своим.",
  ...
  const startBattle = useCallback(
    (planetId: PlanetId) => {
      setState((prev) => {
        ...
        if (prev.expeditions.some((e) => e.shipId === selectedShipId)) return prev;
  ...
  const attackBattle = useCallback(() => {
    setState((prev) => {
      if (!prev.battle) return prev;
      const damage = Math.floor(computeBaseShipDamage(prev.fleet) * derived.damageResearchMultiplier);
      if (damage <= 0) return prev;
      const newHP = Math.max(0, prev.battle.currentHP - damage);
      if (newHP === 0) {
        const alien = ALIENS.find((a) => a.planetId === prev.battle!.planetId);
        return {
          ...prev,
          battle: null,
  ...
  const startExpedition = useCallback((expeditionId: ExpeditionId, shipId: ShipId) => {
    setState((prev) => {
      if (prev.battle) return prev;
      if (prev.expeditions.some((e) => e.shipId === shipId)) return prev;
      const ownedShip = prev.fleet.ownedShips.find((s) => s.shipId === shipId);
      ...
      const newSelectedShipId =
        prev.fleet.selectedShipId === shipId ? null : prev.fleet.selectedShipId;
      return {
        ...prev,
