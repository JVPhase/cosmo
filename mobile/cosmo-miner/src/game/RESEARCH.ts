...
export function computeResearchEffects(research: ResearchState): ComputedResearchEffects {
  ...
  for (const node of RESEARCH) {
    ...
    else if (effect.type === "passiveMultiplier") passiveMultiplierBonus += effect.value;
    else if (effect.type === "metalDropBonus") metalDropBonus += effect.value;
    else if (effect.type === "battleTimerBonus") battleTimerBonus += effect.value;
    else if (effect.type === "damageMultiplier") damageMultiplierBonus += effect.value;
  }

  return { clickMultiplierBonus, passiveMultiplierBonus, metalDropBonus, battleTimerBonus, damageMultiplierBonus };
}
