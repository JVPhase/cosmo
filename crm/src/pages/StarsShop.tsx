...
export default function StarsShopPage() {
  ...
  useEffect(() => {
    ...
    setBusy(true);
    apiFetch<StarsShopItemDetail>(
      `/crm/stars-shop/items/${encodeURIComponent(itemId)}`,
    )
      .then((item) => {
        setDetail(item);
        setForm({
          ...
          priceCredits:
            item.priceCredits === null ? '' : String(item.priceCredits),
  ...
  async function saveItem() {
    ...
    const payload = {
      ...
      priceCredits:
        form.priceCredits.trim() === '' ? null : Number(form.priceCredits),
    ...
    try {
      const saved = isCreate
        ...
        : await apiFetch<StarsShopItemDetail>(
            `/crm/stars-shop/items/${encodeURIComponent(itemId!)}`,
            {
              method: 'PUT',
              body: JSON.stringify(payload),
      ...
      setForm({
        ...
        priceCredits:
          saved.priceCredits === null ? '' : String(saved.priceCredits),
      ...
      loadItems();
      if (isCreate) {
        navigate(`/stars-shop/${encodeURIComponent(saved.id)}`, {
          replace: true,
        });
      }
    } catch (e) {
        ...
        <header className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-crm-ink/60">
              Здесь редактируются товары из таблицы{' '}
              <code className="text-xs">ShopItem</code> с ценой в Stars.
        ...
        <Card className="border-crm-ink/10 bg-white/80">
          <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2">
            <div>
              ...
              <CardDescription>
                Активные и скрытые позиции Telegram-магазина.
              </CardDescription>
            </div>
              ...
              <TableBody>
                  ...
                  <TableRow key={item.id}>
                    ...
                    <TableCell>
                      {item.isActive ? (
                        ...
                        <Badge variant="cool">inactive</Badge>
                      )}
                    </TableCell>
                  </TableRow>
                ))}
                {!items.length && (
                  <TableRow>
                    <TableCell
                      colSpan={6}
                      className="text-center text-crm-ink/50"
                    >
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <h1 className="font-display text-2xl font-semibold">
          {isCreate ? (
            ...
            <span className="font-mono text-lg">{form.id}</span>
          )}
        </h1>
        {form.isActive ? (
          <Badge>active</Badge>
      ...
      <Card className="border-crm-ink/10 bg-white/80">
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <CardDescription>
              Изменение товара влияет на новые покупки. Уже созданные инвойсы
              исполняются по snapshot версии.
            </CardDescription>
          </div>
